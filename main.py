...
def save_item(name,owner_id,desc,photo,price=0,available=True,item_id=None) -> int:
    if item_id is None:
        item_id = item_id = 1 if (len(itens.keys()) == 0) else max(itens.keys())+1
    item_instance = Item(item_id,name,owner_id,price,desc,photo,available)
    itens[item_instance.id] = item_instance
    users[owner_id].itens[item_id] = item_instance
    return item_id
...
def make_request(item_id,interested_id,supplier_id,state='open'):
    new_request = Request(None,state,item_id,interested_id,supplier_id)
...
def confirmation(): 
    if request.method == 'POST': 
        ...
        resp.set_cookie('email',email)
        # TODO generate e-mail veriication
...
def item_request():
    ...
    if item_id not in user.requests_made.keys():
        make_request(item_id,user.id,seller.id,"payment-pending")
    if user.requests_made[item_id].state == "payment-pending":
...
def payment():
    ...
    users[user_id].requests_made[item_id].state = "open"
    return render_template("payment.html")

@app.route('/payment/qr',methods=['GET'])
def paymentqr():
    item_id = request.args["item_id"]
    user_id = request.args["user_id"]
    seller_id = request.args["seller_id"]
    qr = qrcode.make(URL+url_for("payment",
                        item_id=item_id,
                        user_id=user_id,
                        seller_id=seller_id))
    qr_name = str(item_id)+"_"+str(user_id)+".jpg"
    qr_file = os.path.join(DATA_DIR,BACKUP_DIR+str(BACKUP_COUNTER),QR_DIR,qr_name)
    qr.save(qr_file)
...
def evaluate_publication():
    ...
    photo = request.files['photo']
    item_id = 1 if (len(itens.keys()) == 0) else max(itens.keys())+1
    print(item_id)
    photo_name = str(item_id)+".jpg"
    photo.save(os.path.join(DATA_DIR,BACKUP_DIR+str(BACKUP_COUNTER),PHOTOS_DIR,photo_name))
    user = users[emailsearch[email]]
    ...
    if not verified:
        ...
        resp = render_template("error.html",error=output)
        return resp
    new_item_index = save_item(name,user.id,desc,photo_name,price)
    new_item = itens[new_item_index] 
...
def user():
    ...
    if (not err and requested_user_id == user.id):
        active_user=True
    print([i.id for i in requested_user.itens.values()])
...
if __name__ == '__main__':
    ...
    qr_path = os.path.join(data_path,QR_DIR)
